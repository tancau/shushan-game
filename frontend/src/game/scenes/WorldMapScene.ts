...
export class WorldMapScene extends BaseScene {
  ...
  private currentLocation: string = '峨眉山脚'
  ...
  private setupBackground(): void {
    if (this.textures.exists('bg-worldmap')) {
      this.createBackgroundWithTexture('bg-worldmap')
    } else {
  ...
  private createTitle(): void {
    const title = this.createText(this.centerX, 40, '世界地图', { fontSize: '32px', color: '#FFD700', strokeThickness: 6 })
    title.setOrigin(0.5)
  }

  private createLocations(): void {
    this.locations = [
      { name: '峨眉山', x: 300, y: 250, danger: 1, icon: '🏯', unlocked: true },
      { name: '剑冢', x: 600, y: 200, danger: 4, icon: '💫', unlocked: true },
      { name: '昆仑山', x: 900, y: 150, danger: 3, icon: '🏔️', unlocked: true },
      { name: '成都府', x: 350, y: 400, danger: 0, icon: '🌸', unlocked: true },
      { name: '青城山', x: 700, y: 380, danger: 2, icon: '🏔️', unlocked: true },
      { name: '蜀山', x: 500, y: 300, danger: 5, icon: '⚔️', unlocked: false }
    ]
  ...
  private createLocationMarker(loc: Location): void {
    ...
    // 地点背景
    const bg = this.add.circle(0, 0, 40, loc.unlocked ? COLORS.BG_MEDIUM : 0x333333, 0.8)
    bg.setStrokeStyle(2, loc.unlocked ? COLORS.PRIMARY_GOLD : 0x666666)

    // 地点名称
    const nameText = this.createText(0, -60, loc.name, { fontSize: '16px', color: loc.unlocked ? '#FFFFFF' : '#666666' })
    nameText.setOrigin(0.5)

    // 危险等级
    const dangerStars = '★'.repeat(loc.danger) + '☆'.repeat(5 - loc.danger)
    const dangerText = this.createText(0, 60, `[危险: ${dangerStars}]`, { fontSize: '12px', color: '#FFA726' })
  ...
  private createPlayerMarker(): void {
    ...
    // 你在这里文字
    const label = this.createText(0, -30, '你在这里', { fontSize: '12px', color: '#4FC3F7' })
    label.setOrigin(0.5)
  ...
  private createLocationInfo(): void {
    ...
    const panel = this.add.rectangle(this.centerX, panelY, 600, 80, COLORS.BG_MEDIUM, 0.8)
    panel.setStrokeStyle(2, COLORS.PRIMARY_GOLD)
    
    const infoText = this.createText(this.centerX, panelY - 20, `当前位置: ${this.currentLocation}`, { fontSize: '18px', color: '#FFD700' })
    infoText.setOrigin(0.5)
    
    const hint = this.createText(this.centerX, panelY + 15, '点击地点可移动', { fontSize: '14px', color: '#4FC3F7' })
    hint.setOrigin(0.5)
  }
